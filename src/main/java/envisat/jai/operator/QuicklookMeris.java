...
/**
 * This render image factory is dedicated to the preparation of the MERIS quicklook operator.
 */
public class QuicklookMeris implements RenderedImageFactory
{
   private static final Logger LOGGER = Logger.getLogger(QuicklookMeris.class);

   /**
    * Creates a new instance of <code>QuicklookMerisOpImage</code> in the
   ...
   public RenderedImage create(ParameterBlock param, RenderingHints hints)
   {
      ...
      // Here, MERIS histogram equalization is performed in the same way as for S3 OLCI data.
...
//                  The MERIS band data are already provided in terms of reflectances. Thus, only pixel corrections are applied.
                  double reflectance = band.getData().getSample(j, i, 0) * pc.scale + pc.offset;
    ...
    * @param sources the Envisat MERIS datasources.
   ...
   /**
    * Extracts the solar flux from input Envisat MERIS datasources.
    * @param sources the Envisat MERIS datasources.
   ...
   /**
    * Extracts the sun angles from input Envisat MERIS datasources.
    * @param sources the Envisat MERIS datasources.
